...
1. Java 内存模型（**JMM**）;
2. 重排序与 happens-before 原则了解吗? 在java指令的前后插入内存屏障禁止指令执行被优化重排；happens-before：操作1必须在操作2之前，那么1对2可见，1先执行吗，如果是，结果无影响则不限制
3. volatile 关键字的作用; 可见性，重排序
4. 说说 synchronized 关键字和 volatile 关键字的区别; 限制线程访问，一个从可见性，指令排序来应对并发
5. ......

#### ThreadLocal

1. 有啥用（解决了什么问题）？怎么用？ 线程间数据共享，get set
2. 原理了解吗？ 维护map 线程是key 值是value
3. 内存泄露问题了解吗？ 线程被回收产生null key 却有值的value，记得调用remove方法
